...
from audit_lib.config.run import run_audit_lib, create_module_dic
...
def runSysAuditTool(category_selected='all', subcategories={}, password=None):
    """
    This function will be removed, still there for compatibility with other tools
    Everything is on the config/run.py file
    """
    for pwd_dic in run_audit_lib(category_selected=category_selected, subcategories=subcategories, password=password):
        yield pwd_dic
...
if __name__ == '__main__':
    ...
    for r in runSysAuditTool(category_selected=category, subcategories=subcategories, password=args.get('password', None)):
        pass
