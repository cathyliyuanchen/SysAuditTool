...
class MavenRepositories(ModuleInfo):
    ...
    def run(self):
        ...
        to allow "SysAuditTool run initiator" the use the encryption parameter associated with the version of Maven because
        encryption parameters can change between version of Maven.

        - "SysAuditTool run initiator" can also use the encrypted password and the master password "AS IS"
