...
from audit_lib.config.change_privileges import list_sids, rev2self, impersonate_sid_long_handle
from audit_lib.config.users import get_user_list_on_filesystem, set_env_variables, get_username_winapi
from audit_lib.config.dpapi_structure import SystemDpapi, are_masterkeys_retrieved
from audit_lib.config.execute_cmd import save_hives, delete_hives
from audit_lib.config.write_output import print_debug, StandardOutput
from audit_lib.config.constant import constant
from audit_lib.config.manage_modules import get_categories, get_modules
...
def run_audit_lib(category_selected='all', subcategories={}, password=None):
    """
    Execution Workflow:
    - If admin:
