...
from audit_lib.config.write_output import write_in_file, StandardOutput
from audit_lib.config.manage_modules import get_categories
from audit_lib.config.constant import constant
from audit_lib.config.run import create_module_dic, run_audit_lib
...
def clean_args(arg):
    ...
    """
    for i in ['output', 'write_normal', 'write_json', 'write_all', 'verbose', 'auditType', 'quiet', 'mode']:
        try:
            del arg[i]
        except Exception:
            pass
...
def runSysAuditTool(category_selected='all', subcategories={}):
    """
    This function will be removed, still there for compatibility with other tools
    Everything is on the config/run.py file
    """
    for pwd_dic in run_audit_lib(category_selected=category_selected, subcategories=subcategories):
        yield pwd_dic


if __name__ == '__main__':
    # Pre-process legacy flags to avoid argparse option conflicts:
    # - `--mode=standard` -> behave like subcommand `all`
    # - `--mode=<category>` -> behave like subcommand `<category>` when valid
    # - `--check-env` -> behave like subcommand `browsers`
    if len(sys.argv) > 1:
        raw = sys.argv[1:]
        mapped = None
        i = 0
        ...
        if mapped:
            sys.argv = [sys.argv[0], mapped] + new_args
    ...
    # If --mode provided, map it to auditType (maintain compatibility with subcommands)
    if args.get('mode'):
        if args['mode'] == 'standard':
            args['auditType'] = 'all'
        else:
            args['auditType'] = args['mode']
    ...
    for run in runSysAuditTool(category, subcategories):
        pass
